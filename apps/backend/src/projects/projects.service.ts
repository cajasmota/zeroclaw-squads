...
export class ProjectsService {
  ...
  private encryptConfig(config: CreateProjectDto['config']): any {
    if (!config) return {};
    const enc = { ...config };
    if (enc.slackToken) enc.slackToken = this.encryption.encrypt(enc.slackToken);
    if (enc.githubApp?.privateKey) {
      enc.githubApp = { ...enc.githubApp, privateKey: this.encryption.encrypt(enc.githubApp.privateKey) };
    }
    if (enc.githubApp?.webhookSecret) {
      enc.githubApp = { ...enc.githubApp, webhookSecret: this.encryption.encrypt(enc.githubApp.webhookSecret) };
    }
    if (enc.llmKeys?.openai) enc.llmKeys = { ...enc.llmKeys, openai: this.encryption.encrypt(enc.llmKeys.openai) };
    if (enc.llmKeys?.anthropic) enc.llmKeys = { ...enc.llmKeys, anthropic: this.encryption.encrypt(enc.llmKeys.anthropic) };
    if (enc.llmKeys?.google) enc.llmKeys = { ...enc.llmKeys, google: this.encryption.encrypt(enc.llmKeys.google) };
    return enc;
  }
  ...
  private sanitizeProject(project: any): any {
    ...
    if (p.config) {
      ...
      if (p.config.githubApp?.privateKey) p.config.githubApp = { ...p.config.githubApp, privateKey: '[encrypted]' };
      if (p.config.githubApp?.webhookSecret) p.config.githubApp = { ...p.config.githubApp, webhookSecret: '[encrypted]' };
      if (p.config.llmKeys?.openai) p.config.llmKeys = { ...p.config.llmKeys, openai: '[encrypted]' };
      if (p.config.llmKeys?.anthropic) p.config.llmKeys = { ...p.config.llmKeys, anthropic: '[encrypted]' };
      if (p.config.llmKeys?.google) p.config.llmKeys = { ...p.config.llmKeys, google: '[encrypted]' };
    }
    return p;
  }
  ...
  async findById(tenantId: Types.ObjectId, id: string) {
    const project = await this.projectModel
      ...
      .populate('roles.librarian roles.architect roles.pm roles.developer roles.reviewer roles.optional')
      .lean()
      .exec();
  ...
  async create(tenantId: Types.ObjectId, dto: CreateProjectDto) {
    ...
    this.eventEmitter.emit('project.created', { projectId: project._id.toString(), tenantId: tenantId.toString() });
