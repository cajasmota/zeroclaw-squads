...
function makeSettings(overrides: Partial<any> = {}) {
  return {
    ...
    llmKeys: { openai: 'enc:global-openai', anthropic: 'enc:global-anthropic', google: 'enc:global-google' },
...
describe('GlobalSettingsService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getModelToken(GlobalSettings.name), useValue: settingsModel },
  ...
  describe('get()', () => {
    it('should return existing settings', async () => {
      ...
      settingsModel.findOne.mockReturnValue({ exec: () => Promise.resolve(doc) });
    ...
    it('should create settings if none exist', async () => {
      const doc = makeSettings();
      settingsModel.findOne.mockReturnValue({ exec: () => Promise.resolve(null) });
  ...
  describe('update()', () => {
    it('should encrypt llmKeys before saving', async () => {
      const doc = makeSettings();
      settingsModel.findOneAndUpdate.mockReturnValue({ exec: () => Promise.resolve(doc) });
      await service.update(tenantId, { llmKeys: { openai: 'raw-key', anthropic: '', google: '' } });
    ...
    it('should not re-encrypt already masked keys', async () => {
      const doc = makeSettings();
      settingsModel.findOneAndUpdate.mockReturnValue({ exec: () => Promise.resolve(doc) });
      await service.update(tenantId, { llmKeys: { openai: '[encrypted]', anthropic: '', google: '' } });
  ...
  describe('resolveLlmKeys()', () => {
    ...
    it('should return only enabled providers', async () => {
      const doc = makeSettings({
        providers: { openai: false, anthropic: false, google: false, ollama: true },
      });
      settingsModel.findOne.mockReturnValue({ exec: () => Promise.resolve(doc) });
      const result = await service.resolveLlmKeys(tenantId, null);
