...
export class LibrarianMcpService {
  ...
  async getTypeDefinition(typeName: string): Promise<McpToolResult> {
    try {
      const resp = await axios.get(
        `${this.parserUrl}/types/${encodeURIComponent(typeName)}`,
        { timeout: 10000 },
      );
      ...
      return {
        ...
        result: `Type not found: ${typeName}`,
      };
    }
  }
  ...
  async getComponentSample(componentName: string): Promise<McpToolResult> {
    try {
      const resp = await axios.get(
        `${this.parserUrl}/samples/${encodeURIComponent(componentName)}`,
        { timeout: 10000 },
      );
      return {
        tool: 'get_component_sample',
        result: JSON.stringify(resp.data),
      };
    } catch {
      return {
        tool: 'get_component_sample',
        result: `Sample not found: ${componentName}`,
      };
    }
  }
  ...
  async analyzeImpact(filePath: string): Promise<McpToolResult> {
    try {
      ...
      return {
        tool: 'analyze_impact',
        result: `Impact analysis unavailable for: ${filePath}`,
      };
    }
  }

  async checkConventionCompliance(
    filePath: string,
    content: string,
    ...
    try {
      ...
      return {
        tool: 'check_convention_compliance',
        result: JSON.stringify(resp.data),
      };
    } catch {
      return {
        tool: 'check_convention_compliance',
        result: 'Compliance check unavailable',
      };
    }
  }
  ...
  async handleToolCall(
    ...
    switch (toolName) {
      case 'find_logic':
        return this.findLogic(params.query);
      case 'ask_question':
        return this.askQuestion(params.question);
      case 'get_type_definition':
        return this.getTypeDefinition(params.typeName);
      case 'get_component_sample':
        return this.getComponentSample(params.componentName);
      case 'analyze_impact':
        return this.analyzeImpact(params.filePath);
      case 'check_convention_compliance':
        return this.checkConventionCompliance(params.filePath, params.content);
      default:
